...
export const generateWebsite = async (req, res) => {
  try {
    ...
    if (user.credits < 50) {
      return res.status(400).json({
        success: false,
        message: "You don't have enough credit to generate website",
      });
    }

    // Correct prompt creation
    ...
    for (let i = 0; i < 2 && !parsed; i++) {
      ...
      parsed = await extractJson(aiText);

      if (!parsed) {
        raw = await generateResponse(finalPrompt + "\n\nRETURN ONLY RAW JSON.");

        const retryText = raw?.choices?.[0]?.message?.content;

        parsed = await extractJson(retryText);
      }
    }
    ...
    console.log("Error in generate website controller: ", error);
...
export const getWebsiteById = async (req, res) => {
  try {
    ...
    console.log("Error in getById website: ", error);
...
export const changes = async (req, res) => {
  try {
    ...
    if (user.credits < 25) {
      return res.status(400).json({
        message: "Not enough credits available to generate a website. Please add credits to continue.",
      });
    }
